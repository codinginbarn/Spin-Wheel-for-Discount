		...
		const SPIN_EXPIRY_TIME_SECONDS = 45;
		// 86400=1 day in seconds, 2592000=30 days in seconds
		// https://www.timecalculator.net/seconds-to-hours
		// https://www.timecalculator.net/seconds-to-days

		let currentReward = null;
		let spinDisabledUntil = null;
		...
		const rewards = [
			...
			{ label: '10% discount', message: 'This is the message that appears for <b>10% discount</b>', weight: 20 },
			{ label: '0% discount', message: 'This is the message that appears for <b>0% discount</b>', weight: 20 },
			{ label: '50% discount', message: 'This is the message that appears for <b>50% discount</b>', weight: 10 },
		];
		...
		function updateSpinButtonState() {
			...
			if (spinDisabledUntil && currentTime < spinDisabledUntil) {
				spinButton.disabled = true;
			} else {
				spinButton.disabled = false;
				spinDisabledUntil = null;
			}
		}

		window.addEventListener('load', () => {
			const storedSpinDisabledUntil = localStorage.getItem('spinDisabledUntil');
			if (storedSpinDisabledUntil) {
				spinDisabledUntil = parseInt(storedSpinDisabledUntil, 10);
			}
		...
		spinButton.addEventListener('click', () => {
			...
			spinButton.disabled = true;
			const currentTime = new Date().getTime();
			spinDisabledUntil = currentTime + (SPIN_EXPIRY_TIME_SECONDS * 1000);
			localStorage.setItem('spinDisabledUntil', spinDisabledUntil);
		});

		rewardButton.addEventListener('click', getReward);

		setInterval(updateSpinButtonState, 1000);